        ...
        to-green-300 pt-24 pb-7'>
          <div className='grid items-center grid-cols-3 justify-items-center'>
            ...
            {links?.map((list, index) => (
              <ul key={index} className='grid items center justify-items-center
              gap-1'>
                {list?.map((val, index) => (
                  <li key={index} className='font-mono text-sm sm:text-xs'>{val.link}</li>
                ))}
              </ul>
            ))}
          </div>
          <div className='w-7/12 lg:w-[95vw] m-auto mt-9'>
            ...
            <div className='flex items-center justify-between px-7 md:px-0 md:gap-5 md:flex-col-reverse'>
              <p className='text-sm md:text-center'>Copyright<sup className='text-base font-bold'>&copy;</sup> All Rights Reserved 2023
               <span className='font-semibold'> Ksonsoft Developers</span></p>
              <div className='flex items-center gap-3'>
                {sociallinks?.map((val, index) => (
                  <img key={index} src={val.icon} alt='social/icon' className='w-5 h-5' />
                ))}
              </div>
            </div>
          </div>
